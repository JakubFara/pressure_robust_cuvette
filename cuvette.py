...
bc_x_outer = fd.DirichletBC(V.sub(0).sub(0), expr_x_outer, 1)
bc_y_outer = fd.DirichletBC(V.sub(0).sub(1), expr_y_outer, 1)
bc_x_inner = fd.DirichletBC(V.sub(0).sub(0), expr_x_inner, 2)
bc_y_inner = fd.DirichletBC(V.sub(0).sub(1), expr_y_inner, 2)
...
problem = fd.NonlinearVariationalProblem(form, w, bcs=bcs, J=J)
